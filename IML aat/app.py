...
def get_preprocessor():
    try:
        return CarPreprocessor(data_path="data/cars.csv")
    except Exception as e:
        st.error(f"Error loading preprocessor: {e}. Attempting to run generate_data.py first.")
        # Execute generate_data script programmatically if needed
        import subprocess
...
st.markdown("<p class='sub-title'>Advanced Machine Learning Assisted Automobile Discovery (K-Nearest Neighbors Algorithm)</p>", unsafe_allow_html=True)
...
st.sidebar.markdown("---")
st.sidebar.info(
    "**Project Context:** Final Year Engineering Mini Project (Machine Learning / Pattern Recognition Stream)"
)
...
with tab1:
    ...
    st.write(
        f"The K-Nearest Neighbors model evaluated **{len(df)}** vehicles using **{distance_metric}** under **{recommendation_mode}** and identified the following top matches based on your inputs:"
    )
    ...
    if recommendations.empty:
        ...
        with chart_col2:
            ...
            st.write(
                ...
                "The **Distance** indicates absolute spatial vector difference from your scaled input vector. A smaller distance represents a more perfect match."
            )
